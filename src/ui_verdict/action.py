...
def parse_action(action: str) -> ParsedAction:
    ...
    if cmd == "click":
        if len(parts) < 2:
            raise ActionParseError("click action requires coordinates: click:500,300")
        try:
    ...
    if cmd == "rightclick":
        if len(parts) < 2:
            raise ActionParseError("rightclick action requires coordinates: rightclick:500,300")
        try:
            coords = parts[1].split(",")
            x, y = int(coords[0]), int(coords[1])
        except (ValueError, IndexError):
    ...
    raise ActionParseError(f"Unknown action type: {cmd}. Use key/click/rightclick/type/wait")
...
def execute_action(action: str | ParsedAction) -> None:
    ...
    elif parsed.action_type == ActionType.CLICK:
        vm_click(parsed.x, parsed.y, "left")
    elif parsed.action_type == ActionType.RIGHTCLICK:
        vm_click(parsed.x, parsed.y, "right")
    elif parsed.action_type == ActionType.TYPE:
        vm_type(parsed.text)
    elif parsed.action_type == ActionType.WAIT:
